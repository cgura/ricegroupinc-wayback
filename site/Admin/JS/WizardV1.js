...
//szFileName = szFileName2;
		...
		//2/20/2007 - Added a random number generator to the name section to enable multiple sites to open the same content editor at the same time
		var random = Math.floor(Math.random()*999999999) + szFileName.substring(0,szFileName.length-2);
        ...
        window.open("/apps/ContentEditor/ContentEditor.aspx?buttID=" + szbuttID + "&filename=" + szFileName + "&LanguageList=" + szLanguageList + "&" + szLoadLink, random, "top="+ intTop +",left="+ intLeft +",status=1,resizable=1,scrollbars=1,toolbar=0,width=800,height=" + nHeight)
		}
...
function loadVar()
{
    if (document.getElementById('wizard') != undefined) {
		document.getElementById('wizard').SetVariable("eow", "true");
		}
}
function loadVars()
{
	if (document.getElementById('wizard') != undefined)
		document.getElementById('wizard').SetVariable("eopopup", "true");
}
function sendVar(szFileName) {
    if (document.getElementById('wizard') != undefined)
    {
       document.getElementById('wizard').SetVariable("uploadComplete", "true," + szFileName);
    }        
    else if (document.getElementById('wizard2') != undefined)
        document.getElementById('wizard2').SetVariable("uploadComplete", "true," + szFileName);
    else if (document.getElementById('WizardModule') != undefined) {
        console.log("WizardModuleExists");
        document.getElementById('WizardModule').SetVariable("uploadComplete", "true," + szFileName);
    }
}
...
{
	var intTop = (screen.availHeight - 650) / 2;
	var intLeft = (screen.availWidth - 770) / 2;	
	window.open("/Apps/InProductUpgrades/DomainControl.aspx", "ManageDomains", "toolbar=no,  top=" + intTop + ", left=" + intLeft + ", location=no, directories=no, menubar=no, status=no, scrollbars=yes, resizable=yes, width=760, height=650");
}
function OpenAddDomains()
{
	var intTop = (screen.availHeight - 650) / 2;
	var intLeft = (screen.availWidth - 770) / 2;	
	window.open("/Apps/InProductUpgrades/DomainControl.aspx?Load=AddDomains", "ManageDomains", "toolbar=no,  top=" + intTop + ", left=" + intLeft + ", location=no, directories=no, menubar=no, status=no, scrollbars=yes, resizable=yes, width=760, height=650");
	//window.open("/Apps/InProductUpgrades/DomainControl.aspx", "ManageDomains", "toolbar=no,  top=" + intTop + ", left=" + intLeft + ", location=no, directories=no, menubar=no, status=no, scrollbars=yes, resizable=yes, width=750, height=550");
}
function OpenDomainTransfer()
{
	OpenWindowCentered("/Apps/DomainManager/transfer.aspx", "TransferDomains", 650, 770, 1);
}
...
function loadBizManLoader() {
	loadBizManLoaderWithParams("");
}

function loadBizManLoaderWithParams( params ) {
	var nWidth = 850;
	var nHeight = 760;
	...
	var intLeft = (screen.availWidth - nWidth) / 2;
	if ( params != "" )
		window.open("/Admin/BizManLoader.aspx?" + params, "bizmanwin", "toolbar=no,  top=" + intTop + ", left=" + intLeft + ", location=no, directories=no, menubar=no, status=no, scrollbars=no, resizable=1, width=" + nWidth + ", height=" + nHeight + "");
	else
		window.open("/Admin/BizManLoader.aspx", "bizmanwin", "toolbar=no,  top=" + intTop + ", left=" + intLeft + ", location=no, directories=no, menubar=no, status=no, scrollbars=no, resizable=1, width=" + nWidth + ", height=" + nHeight + "");
}
